...
public:
	...
	int SearchFirstListinList(List list) // Поиск первого вхождения другого списка в список.
	{
		...
		if (NotEmpty) { // Если наш список не пустой
			...
			while (current != NULL && current->next != NULL && !exit) { // Если список этот элемент и последующий не равны 0.
				comparise = false; 
...
void OtherMethods(List* list1, List* list2) {
	...
	cout << "1.Получение элемента по индексу.\n"
		...
		<< "4.Поиск первого вхождения другого списка в список.\n"
		<< "5.Поиск последнего вхождения другого списка в список.\n"
